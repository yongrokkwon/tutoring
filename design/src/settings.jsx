/* Area 4 — /settings page (interactive) */
...
function CustomTimesCard({ times, onDelete }) {
  return (
    <div className="ap-card" style={{ padding: "8px 8px" }}>
      ...
      <div style={{ padding: "12px 16px 8px" }}>
        <a href="./settings-times-add.html" style={{
          display: "block",
          textAlign: "center",
...
function SettingsApp() {
  ...
  const [times, setTimes] = React.useState(INITIAL_CUSTOM_TIMES);
  ...
  const handleSave = () => {
    showToast(`저장됨 · 주기: ${FREQ_OPTIONS.find(o => o.id === mode).label}`);
  };

  const handleTest = () => {
    showToast("발송 테스트 — Discord 채널을 확인해주세요");
  };
      ...
      <div className="ap-page-inner--narrow">
        ...
        {mode === "custom" && (
          <section style={{ marginBottom: 36 }}>
            ...
            />
            <CustomTimesCard times={times} onDelete={handleDeleteTime} />
          </section>
        )}
        ...
        <div style={{
          ...
          <button className="ap-btn-primary" style={{ minWidth: 120 }} onClick={handleSave}>저장</button>
        </div>
        ...
        <section id="test-send">
          ...
          <div className="ap-card" style={{ padding: 28 }}>
            ...
            <button className="ap-btn-secondary" onClick={handleTest}>발송 테스트</button>
          </div>
        </section>
