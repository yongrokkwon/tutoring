...
class ParsedNotice:
    duid: int
    title: str
    category_id: Optional[int]  # parser 는 항상 None. classifier 가 채움.
    category_name: str
    author: str
    posted_date: str
    modified_date: str
...
def parse(html: str) -> list[ParsedNotice]:
    ...
    for li in list_box.find_all("li", recursive=False):
        ...
        if row is None:
            continue
        # 같은 DUID 가 고정/일반 양쪽에 노출될 수 있다 — 고정을 우선해 유지.
...
def _parse_row(li: Tag) -> Optional[ParsedNotice]:
...
def _extract_title(anchor: Tag, category_tag: Tag) -> str:
    # descendants 로 순회하면 주석 텍스트·카테고리 라벨이 끼어든다.
    # anchor 의 직접 children 만 순회 + Comment / ico-* span / category_tag skip.
    parts: list[str] = []
